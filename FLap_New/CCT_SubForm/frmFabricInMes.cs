...
using FLap_New.SubForm.Mes_ChildForm;
using OfficeOpenXml;
...
namespace FLap_New.SubForm
{
    public partial class frmFabricInMes : Form
    {
        ...
        private void btnMesPrint_Click(object sender, EventArgs e)
        {
            ...
            //    }
            //}
            var selectedRows = dgvMesData.Rows.Cast<DataGridViewRow>()
               .Where(row => Convert.ToBoolean(row.Cells[0].Value) == true)
              .Select(row => new FabricInfoDTO
              {
                  So = row.Cells["So"].Value.ToString(),
                  Roll = row.Cells["Roll"].Value.ToString(),
                  Batch = row.Cells["Batch_No"].Value.ToString(),
                  Length = Convert.ToDecimal(row.Cells["Length"].Value),
                  Color = row.Cells["Color Name"].Value.ToString(),
            ...
            if (selectedRows.Any())
            {
                var formQR = new FormQR();
                // Đặt form cha (formQR) to bằng kích thước màn hình
                formQR.WindowState = FormWindowState.Maximized; // Phóng to form cha

                formQR.Show();
                formQR.LoadData(selectedRows);
            }
            else
            {
                MessageBox.Show("Bạn cần chọn bản ghi để tiếp tục");
            }
        }
    }
